...
const DashboardContent = () => {
  ...
  const [last6MonthsExpenses, setLast6MonthsExpenses] = useState<{ label: string; value: number }[]>([]);
  const [categoryExpenses, setCategoryExpenses] = useState<{ label: string; value: number }[]>([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        await checkAndInsertAlerts();
        const [expenses, categories, subcategories] = await Promise.all([
          getLastSixMonthsExpenses(),
          getMonthlyExpenseByCategory(),
          getMonthlySubcategoriesGroupedByCategory(),
        ]);
        setLast6MonthsExpenses(expenses);
        setCategoryExpenses(categories);
        setSubcategoryMap(subcategories);
      } catch (err) {
    ...
    <Box bg="gray.100" minH="100vh">
      ...
      <Flex>
        ...
        >
          ...
          <ExpenseChart expenses={last6MonthsExpenses} />
          <CategoryPieChart
            data={categoryExpenses.map(({ label, value }) => ({
              category: label,
              amount: value,
            }))}
            subcategoryMap={subcategoryMap}
